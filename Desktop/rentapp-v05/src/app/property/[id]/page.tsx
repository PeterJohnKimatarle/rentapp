        ...
        >
          ...
          >
            ...
            {(() => {
              ...
              // Check if description looks auto-generated (format: "A [Type] located in [Location]")
              const isAutoGenerated = description && /^A\s+\w+\s+\([^)]+\)\s+located\s+in\s+[^,]+,\s+[^.]+\.$/.test(description);

              console.log('Description debug:', {
                descriptionValue: description,
                isAutoGenerated,
                finalCondition: description && !isAutoGenerated
              });

              return description && !isAutoGenerated ? (
              <div className="text-gray-700 leading-relaxed whitespace-pre-wrap py-1.5">
                {description}
              </div>
              ) : (
              <div className="text-gray-600 text-center py-1.5">
