...
export default function PropertyCard({ property, onBookmarkClick, showMinusIcon = false, hideBookmark = false, showEditImageIcon = false, onEditImageClick, onStatusChange, onEditClick, onManageStart, isActiveProperty = false, showBookmarkConfirmation = true, onApplyStagedChanges, stagedImageCount, renderAfterUpdated, isRemovedBookmark = false, showNotesButton = false, showClosedButton = false }: PropertyCardProps) {
  ...
  usePreventScroll(showBookmarkPopup || showRemoveBookmarkPopup || showSharePopup || showActionPopup || showBookingModal || showStatusModal || showThreeDotsModal || showNotesModal || showInfoModal);
  ...
  const handleImageClick = () => {
    ...
    // Mark this property as the last viewed (for blue dot indicator)
    if (property.id) {
          ...
          >
            ...
            {!hideBookmark && user && user.role === 'staff' && user.isApproved && (
              <div className="mt-2 flex gap-2 items-center" onClick={(e) => e.stopPropagation()}>
                ...
                >
                  {(showNotesButton || isPinged) && hasNotes && (
                  ...
                  {(showNotesButton || isPinged) ? (
                    ...
                    <>
                      {(showClosedButton || isClosed) && <Check size={18} className="text-white" strokeWidth={3} />}
                      <span className="select-none" style={{ WebkitUserSelect: 'none', MozUserSelect: 'none', msUserSelect: 'none', userSelect: 'none' }}>
                        {(showClosedButton || isClosed) ? 'Closed' : 'Default'}
                      </span>
                    </>
                  )}
                </button>
          ...
          >
            <div className="space-y-2">
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  // Set to Idle (remove from both lists)
                  if (isPinged) {
