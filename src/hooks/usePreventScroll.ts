...
export function usePreventScroll(isOpen: boolean) {
  useEffect(() => {
    ...
    if (isOpen) {
      // Store scroll position and viewport height
      const scrollY = window.scrollY;
      const viewportHeight = window.innerHeight;
      const html = document.documentElement;
      const body = document.body;
      
      // Lock viewport height to prevent address bar show/hide
      const lockViewport = () => {
        // Lock html element - CRITICAL for mobile
      ...
      // Re-lock on resize to maintain viewport height
      const handleResize = () => {
        lockViewport();
      };
      ...
      const preventScroll = (e: Event) => {
        ...
        if (scrollTarget === window || scrollTarget === document || 
            scrollTarget === html || scrollTarget === body ||
            currentTarget === window || currentTarget === document ||
            currentTarget === html || currentTarget === body) {
          e.preventDefault();
          e.stopPropagation();
      ...
      const preventTouchStart = (e: TouchEvent) => {
        ...
        // If touching the overlay or body/html directly, prevent
      ...
      html.addEventListener('scroll', preventScroll, { passive: false, capture: true });
      ...
      return () => {
        if (typeof document !== 'undefined') {
          ...
          htmlEl.removeEventListener('scroll', preventScroll, { capture: true } as EventListenerOptions);
